<template>
  <basic-layout class="h-full">
    ...
    <nut-tabs v-model="value" style="height: calc(100% - 230px)" type="smile" title-scroll direction="vertical">
      <nut-tab-pane v-for="item in videos" :key="item.id" :pane-key="item.id" :title="item.name">
        <nut-cell-group>
          <nut-cell v-for="chi in item.children" :key="chi.id" :title="chi.name" @click="onChangeVideo(chi)" />
        </nut-cell-group>
      </nut-tab-pane>
    </nut-tabs>
  </basic-layout>
